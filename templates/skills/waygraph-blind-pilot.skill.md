...
pre-existing Blocks. `auto --cli --detach` tolerates a project with zero `.block.ts` files -
`auto click/type/goto/press/upload` act on the real page before any Block exists to cover it.
...
1. `auto goto <sessionId> <url>` / `auto click` / `auto type` to explore the real page.
2. `auto dom <sessionId> --mode full --selector <sel>` to read real structure before writing
...
6. Ask the user a clarifying question only when something genuinely can't be resolved from
   ...
   - don't ask about things `auto dom` can just answer.
...
- **`discoverGraph`'s per-file import errors are mostly silent.** A malformed newly-written
  ...
  Block you just wrote doesn't show up after `auto reload`, check `waygraph graph`'s own
...
- **A `<p>`/text-based selector often needs scoping**, not just a class name - a sibling
  ...
  Confirm via `auto dom --mode full` before assuming a class alone is unique enough.
- **Confirm a selector count, not just its first match**, when building anything the menu
  will expand into multiple live rows (an `instanceOptions`-driven Block) - `auto dom`'s
...
- **Prefer the real Pilot overlay over ad hoc status output** when driving a
  `--non-headless` session someone might be watching - it auto-updates on every menu build,
  point a human at the badge/panel instead of re-inventing status reporting.
