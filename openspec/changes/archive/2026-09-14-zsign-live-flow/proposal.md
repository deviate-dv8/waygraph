...
Never implemented (0/7 tasks). Superseded by the real `projects_waygraph/zsign-all`
project, which already does exactly this (register -> verify -> login against the live
stack, via `water-run.mjs` and friends) - see that project's own
`docs/minis-handout.md`. This proposal's `project/zsign/` + `packages/*` workspace
layout also no longer matches this repo's actual shape (single package, no workspace
split). Archived 2026-09-14, not implemented.
