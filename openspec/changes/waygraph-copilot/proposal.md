...
Copilot" roadmap and the stated gate for `1.0.0`: point a real user at a real running app
...
An earlier pass at this proposal wrongly assumed Copilot had to run as a sandboxed,
...
uses. Corrected: Copilot is a **launched session**, exactly like every `waygraph auto`/
...
- **New: a `waygraph copilot` CLI entry point** (`ask "<text>" [--mode narrate|agentic]
...
### New Capabilities
- `waygraph-copilot`: resolves a plain-language ask to a real, reachable Checkpoint/edge
...
- New module (e.g. `src/copilot.ts`): the plain-language resolver, and narrate/agentic
  dispatch built on `AutoSession`.
- `src/cli.ts`: export the ring-rendering primitives (`cycleHighlightRings`, `showRing`, and
  their small helpers) so `src/copilot.ts` can reuse them instead of duplicating; add the new
  `copilot` sub-command.
...
- One in-repo example (`examples/saucedemo` or `templates/scaffold`) wired with a real,
  proven Copilot demo (a real ask, real resolution, real narration, real agentic execution).
